...
c = C(-0.500036, -0.520156)
eps, limit = max(c.re + c.im, 2), 64
umin, umax, vmin, vmax = -2, 1, -1.2, 1.2

def divergira_li(c, m, z):
...
def pripremi_za_crtanje():
    ...
    for x in range(0, width + 1):
        for y in range(0, height + 1):
            z = C(((umax - umin) * x) / width + umin, ((vmax - vmin) * y) / height + vmin)
            k = divergira_li(c, limit, z)
